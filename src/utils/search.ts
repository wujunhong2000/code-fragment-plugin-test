...
// Send search query to google, get answers from stackoverflow 发送搜索查询到谷歌，从stackoverflow得到答案
// then extract and return code results 然后提取并返回代码结果
export async function search(keyword: string): Promise<null | { results: SnippetResult[] }> {
    ...
    const promise = new Promise<{ results: SnippetResult[] }>(async (resolve, reject) => {
        ...
        try {
            ...
            console.log("🚀 ~ file: search.ts:36 ~ promise ~ SnippetExtractors:", SnippetExtractors);
            for (const i in SnippetExtractors) {
                ...
                if (extractor.isEnabled()) {
                    ...
                    for (const y in urls) {
                        ...
                        await console.log("🚀 ~ file: search.ts:45 ~ promise ~ fetchResult:", fetchResult);
                    
                        results = results.concat(extractor.extractSnippets(fetchResult));
                        console.log("🚀 ~ file: search.ts:50 ~ promise ~ results:", results);
        ...
        // When promise resolved, show finished loading for 5 seconds 显示完成加载5秒
        vscode.window.setStatusBarMessage(`StarMoonSnippets: 结束加载 ${results.length} 结果`);
    });

    vscode.window.setStatusBarMessage(`StarMoonSnippets: 等待代码片段查询结果...`, promise);
    return promise;
}
